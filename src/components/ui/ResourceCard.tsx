    ...
    >
      ...
      <div className="p-3 flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <p className="font-normal text-sm text-white group-hover:text-icon transition-colors truncate">
            {resource.titre}
          </p>
          <p className="text-xs text-white/50 capitalize mt-0.5">{resource.type}</p>
        </div>
        <ExternalLink className="h-4 w-4 text-white/40 group-hover:text-icon shrink-0 ml-2" />
      </div>
    </a>
  );
}
